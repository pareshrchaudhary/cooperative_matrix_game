...
class ComediRunner:
    def __init__(self,
        ...
        self.comedi_dir = self.results_dir / "comedi"
        self.env_layout_dir = self.comedi_dir / self.env_layout
        self.policy_dir = self.env_layout_dir / str(self.n_policies)
        self.policy_dir.mkdir(parents=True, exist_ok=True)
        
        self.base_filename = f"comedi_{self.env_layout}_{self.n_policies}"
    ...
    def _custom_loss(self, pool, trainer, payoff_matrix):
        ...
        for idx_policy in range(1, len(pool.policies)):
            cross_play_returns = []
            for jdx_policy in range(1, len(pool.policies)):
                if idx_policy != jdx_policy:
                    j_xp1 = trainer.evaluate(pool.policies[idx_policy], pool.policies[jdx_policy])
                    j_xp2 = trainer.evaluate(pool.policies[jdx_policy], pool.policies[idx_policy])
                    j_xp = j_xp1 + j_xp2
                    cross_play_returns.append(j_xp)
            
            if cross_play_returns:
                max_cross_play = torch.max(torch.stack(cross_play_returns))
                loss_vector[idx_policy] += self.div_factor * max_cross_play

        return loss_vector

    def train_comedi(self, n_seeds):
        pools_trainers = []
        BR_SP_records = []
        BR_XP_records = []
        initial_pools = []
        ...
        for seed in range(n_seeds):
            ...
            pool = PolicyPool(n_policies=self.n_policies, 
                              n_dim=env.n_dim,
                              lr=self.policy_lr,
...
if __name__ == "__main__":
    ...
    for n_policies in policy_sizes:
        print(f"\nTraining Comedi with {n_policies} policies for {n_episodes} episodes and {n_seeds} seeds.")
        env = MatrixGame(layout=env_layout)
        visualizer = Visualizer(env)
        
        runner = ComediRunner(env_layout=env_layout,
        ...
        results = runner.train_comedi(n_seeds=n_seeds)
