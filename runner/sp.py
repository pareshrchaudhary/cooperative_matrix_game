...
class SPRunner:
    def __init__(self,
        ...
        self.sp_dir = self.results_dir / "sp"
        self.env_layout_dir = self.sp_dir / self.env_layout
        self.policy_dir = self.env_layout_dir / str(self.n_policies)
        self.policy_dir.mkdir(parents=True, exist_ok=True)
        
        self.base_filename = f"sp_{self.env_layout}_{self.n_policies}"
    ...
    def train_sp(self, n_seeds):
        ...
        for step in range(self.n_episodes):
            for idx, (pool, trainer, payoff) in enumerate(pools_trainers):
                ...
                BR_SP[idx].append(br_sp_reward.detach())
            ...
            for i in range(n_seeds):
                ...
                BR_XP[i].append(avg_xp_reward)
            
            avg_br = sum(record[-1].item() for record in BR_SP) / n_seeds
            avg_xp = sum(BR_XP[i][-1].item() for i in range(n_seeds)) / n_seeds
            if (step + 1) % 50 == 0:
        ...
        for seed in range(n_seeds):
            ...
            multi_seed_results[seed] = {
                'BR_SP': [r.detach().cpu() for r in BR_SP[seed]],
                'BR_XP': [r.detach().cpu() for r in BR_XP[seed]],
                'pool': pools_trainers[seed][0],
                'BR': br_policy
            }
...
if __name__ == "__main__":
    ...
    for n_policies in policy_sizes:
        print(f"\nTraining SP with {n_policies} policies for {n_episodes} episodes and {n_seeds} seeds.")
        env = MatrixGame(layout=env_layout)
        visualizer = Visualizer(env)
        
        runner = SPRunner(env_layout=env_layout,
                         n_policies=n_policies,
                         n_episodes=n_episodes,
                         policy_lr=policy_lr,
                         policy_std=policy_std)
        
        results = runner.train_sp(n_seeds=n_seeds)
        ...
        pool_fig = visualizer.visualize_pool(results[0]['pool'])
