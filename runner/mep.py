...
class MEPRunner:
    def __init__(self,
        ...
        self.mep_dir = self.results_dir / "mep"
        self.env_layout_dir = self.mep_dir / self.env_layout
        self.policy_dir = self.env_layout_dir / str(self.n_policies)
        self.policy_dir.mkdir(parents=True, exist_ok=True)
        
        self.base_filename = f"mep_{self.env_layout}_{self.n_policies}"
    ...
    def _custom_loss(self, pool, trainer, payoff_matrix):
        ...
        if len(pool.policies) > 1 and self.div_factor > 0:
            probs = []
            for policy in pool.policies[1:]:
                prob_matrix = policy.get_joint_policy().reshape(-1)
                probs.append(prob_matrix)
            
            avg_policy = torch.zeros_like(probs[0])
            for prob in probs:
                avg_policy += prob
            avg_policy /= len(probs)
            
            population_entropy = entropy(avg_policy)
            diversity_loss = -self.div_factor * population_entropy 
            loss_vector[1:] += diversity_loss

        return loss_vector
    
    def train_mep(self, n_seeds):
        pools_trainers = []
        BR_SP_records = []
        BR_XP_records = []
        initial_pools = []
...
if __name__ == "__main__":
    ...
    for n_policies in policy_sizes:
        print(f"\nTraining MEP with {n_policies} policies for {n_episodes} episodes and {n_seeds} seeds.")
        env = MatrixGame(layout=env_layout)
        visualizer = Visualizer(env)
        
        runner = MEPRunner(env_layout=env_layout,
        ...
        results = runner.train_mep(n_seeds=n_seeds)
